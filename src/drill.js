...
const correctAllAudio = new Audio('/touch-shodo/mp3/correct1.mp3');
const correctAudio = new Audio('/touch-shodo/mp3/correct3.mp3');
const incorrectAudio = new Audio('/touch-shodo/mp3/incorrect1.mp3');
const stupidAudio = new Audio('/touch-shodo/mp3/stupid5.mp3');
let prevCanvasSize;
let canvasSize = 140;
let maxWidth = 4;
...
function setScoringButton(problemBox, tegakiPanel, tehonPanel, objects, tegakiPads, word) {
  ...
  scoring.addEventListener('click', function() {
    ...
    getProblemScores(tegakiPanel, tehonPanel, objects, tegakiPads).then(scores => {
      ...
      var clearedKanjis = localStorage.getItem('touch-shodo');
      if (clearedKanjis) {
        var removed = false;
...
function loadProblem(wordYomi) {
  ...
  word.split('').forEach((kanji, pos) => {
    ...
    var object = loadFont(kanji, kanjiId, tegaki, pos, true);
...
function resizeTehonContents() {
  ...
  for (const problem of problems) {
    ...
    [...tegakiBoxes].forEach(tegakiBox => {
      ...
      var ctx = canvas.getContext('2d');
      resizeCanvasSize(canvas, canvasSize);
      var kanjiId = canvas.dataset.id;
      drawFont(canvas, toKanji(kanjiId), kanjiId, true);
    });
    [...tehonBoxes].forEach(tehonBox => {
      ...
      resizeCanvasSize(canvas, canvasSize);
      var kanjiId = canvas.dataset.id;
      drawFont(canvas, toKanji(kanjiId), kanjiId, false);
    });
  }
}
...
function removeAnimations(object) {
  var animations = object.querySelectorAll('path[clip-path]');
...
function fetchJson(grade) {
  return new Promise((resolve, reject) => {
    fetch(`/touch-shodo/data/${grade}.json`)
